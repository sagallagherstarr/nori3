...
def rest_routes(objects, model, route_prefix=""):
    routes = []
    for name, cls in objects.items():
        print('Starting Server - Tornado Rest Framework')
        total_route = "/".join([route_prefix, name.lower()]) # does the right thing if route_prefix is empty string
        total_prefix = "/".join([route_prefix, name]) # for having the name match the route
        
        route = (r'%s/?' % total_route, ResourceHandler, dict(model=model, prefix=name, mtype="list"))
        #print(route)
        print('model:{0} - list and post --> {0}/?'.format(total_route))
        routes.append(route)
        
        route = (r'%s/([0-9a-fA-F]{24,})/?' % total_route,  ResourceHandler, dict(model=model, prefix=name, mtype="detail"))
        #print(route)
        print('model:{0} - get, put, delete --> {0}/([0-9a-fA-F]{{24,}})/?'.format(total_route))
        routes.append(route)
